...
namespace HugoKalidas\LaravelFlexCms\Utilities;


class Utilities
{
    /**The following methods are taken from util.php
     *
...
| (?<=[\xF0-\xF7][\x80-\xBF])[\x80-\xBF](?![\x80-\xBF]) # Short 4 byte sequence (2)
    ...
    public static function strip_excess_space($string)
    {
        return preg_replace('/\s+/', ' ', $string);
    }
     ...
     */
    public static function truncateHtml($text, $length = 150, $ending = '...', $exact = false, $considerHtml = true)
    {
        if ($considerHtml) {
    ...
    public static function mb_convert_case_utf8_variation($s)
    {
    ...
    //incomplete
    public static function csv_to_model_with_relations($file, $model)
    {


        //todo adapt to model

        $arr = static::csv_to_array($file);
        ...
        foreach ($arr as $lineArr) {
            ...
            $lineArr['DISTRITO'] = mb_convert_case($lineArr['DISTRITO'], MB_CASE_TITLE, "UTF-8");
            $lineArr['MUNICIPIO'] = mb_convert_case($lineArr['MUNICIPIO'], MB_CASE_TITLE, "UTF-8");
            $lineArr['FREGUESIA'] = mb_convert_case($lineArr['FREGUESIA'], MB_CASE_TITLE, "UTF-8");

            if ($lineArr['DISTRITO'] != $distrito) {

                $d = Distrito::where("nome", "=", $lineArr['DISTRITO']);
                if (!$d->count()) {
                    $distrito = $lineArr['DISTRITO'];
                    $dist = new Distrito();
                    $dist->nome = $distrito;
                    $dist->save();
                } else {
            ...
            if ($lineArr['MUNICIPIO'] != $concelho) {

                $c = Concelho::where("nome", "=", $lineArr['MUNICIPIO']);
                if (!$c->count()) {
                    $concelho = $lineArr['MUNICIPIO'];
                    $conc = new Concelho();
                    $conc->nome = $concelho;
                    $conc->distritos_id = $dist->id;
                    $conc->save();
                } else {
                    $concelho = $lineArr['MUNICIPIO'];
                    $conc = $c->first();
            ...
            $freg = new Freguesia();
            $freg->nome = $lineArr['FREGUESIA'];
            $freg->concelhos_id = $conc->id;
            $cod = explode("-", $lineArr['CODIGO POSTAL']);
            $freg->codigo_postal_4 = $cod[0];
            $freg->codigo_postal_3 = $cod[1];
    ...
    //incomplete
    public static function csv_to_model($file, $model)
    {


        /**
         * Accepts a csv $file with columns named after the models properties, and the target $model to fill.
         * In each iteration creates, fills and saves instances of the specified model.
         *
         * @param  string $file The system path of the csv file
         * @param  string $model The designation of the target model's class
         *
         * @return void
         */
    ...
    // Todo Extend Eloquent
    public static function csv_seeder($file, $model, $deleteFlag = false)
    {
